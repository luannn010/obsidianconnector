...
export class PgKnowledgeStore implements KnowledgeStore {
      ...
      if (
        ...
        const sql = embedding
          ...
          WITH bm25 AS MATERIALIZED (
            ...
            FROM project_knowledge.search_chunks WHERE content @@@ $3 AND project_id = $1 AND active LIMIT $4
          ), vector AS (
  ...
  async getProjectSyncStatus(
    ...
    try {
      ...
      const [snapshots, projections, queues, conflicts] = await Promise.all([
        this.pool.query(
          'SELECT id,worktree_id,head_commit,dirty_hash,state,activated_at FROM project_knowledge.source_snapshots WHERE project_id=$1 ORDER BY created_at DESC LIMIT 20',
          [row.project_id],
        ),
        this.pool.query(
